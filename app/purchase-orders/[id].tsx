...
export default function PurchaseOrderDetail() {
  ...
  const isOverdue = poFormatters.isOverdue(purchaseOrder.expectedDeliveryDate, purchaseOrder.status);
  const totalWeight = purchaseOrder.items?.reduce((sum, item) => sum + (item.weight || 0), 0) || 0;
  const totalReceivedWeight = purchaseOrder.items?.reduce((sum, item) => sum + (item.receivedWeight || 0), 0) || 0;
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.infoGrid}>
            <View style={styles.infoRow}>
              ...
              <View style={styles.infoItem}>
                ...
                <Text style={styles.infoValue}>{poFormatters.date(purchaseOrder.orderDate)}</Text>
              </View>
              <View style={styles.infoItem}>
                <Text style={styles.infoLabel}>Expected Delivery</Text>
                <Text style={styles.infoValue}>
                  {purchaseOrder.expectedDeliveryDate ? poFormatters.date(purchaseOrder.expectedDeliveryDate) : 'N/A'}
                </Text>
              </View>
            </View>
            ...
            <View style={styles.infoRow}>
              ...
              <View style={styles.infoItem}>
                ...
                <Text style={styles.infoValue}>{(totalWeight || 0).toFixed(2)} Kg</Text>
              </View>
            </View>
            ...
            <View style={styles.infoRow}>
              <View style={styles.infoItem}>
                ...
                <Text style={styles.infoValue}>{purchaseOrder.createdBy || 'Admin'}</Text>
              </View>
              {totalReceivedWeight > 0 && (
                <View style={styles.infoItem}>
                  <Text style={styles.infoLabel}>Received Weight</Text>
                  <Text style={[styles.infoValue, { color: COLORS.success }]}>{(totalReceivedWeight || 0).toFixed(2)} Kg</Text>
                </View>
              )}
            </View>
          </View>
        ...
        {/* Order Items */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Order Items ({purchaseOrder.items?.length || 0})</Text>
          ...
          <View style={styles.table}>
            ...
            {purchaseOrder.items?.map((item, index) => {
              ...
              const completion = item.quantity > 0 ? Math.round((receivedQty / item.quantity) * 100) : 0;
              const itemStatus = completion === 100 ? 'Complete' : completion > 0 ? 'Partial' : 'Draft';
              ...
              return (
                <View key={item._id || index} style={styles.tableRow}>
                  <View style={{ flex: 2 }}>
                    ...
                    {item.itemNotes && (
                      <Text style={styles.itemNotesText}>📝 {item.itemNotes}</Text>
                    )}
                  </View>
                  ...
                  <View style={{ flex: 1.2 }}>
                    <Text style={styles.tableText}>{(item.weight || 0).toFixed(2)}</Text>
                  </View>
                  
                  <View style={{ flex: 1 }}>
                    <View style={[
                      styles.itemStatusBadge,
                      { backgroundColor: itemStatus === 'Complete' ? '#10B981' : itemStatus === 'Partial' ? '#F59E0B' : '#6366F1' }
                    ]}>
...
const styles = StyleSheet.create({
  ...
  itemNotesText: { fontSize: 11, color: '#3B82F6', marginTop: 4, fontStyle: 'italic' },
